...
\begin{description}
  ...
  \item[\cmd{maketitle}] Create the title page using LaTeX's standard \cmd{title}, \cmd{author}, and \cmd{date} values.
    ...
    The article-style \cmd{thanks} command is not supported on title pages.
  ...
  \item[\cmd{makereference}\texttt{[fontsize]\{bibfile\}}] Start a bibliography page using the \texttt{alpha} bibliography style.
    ...
    Use this command instead of calling \cmd{bibliography} directly.
...
Inherited article commands that do not match the foil model report class errors:
\begin{center}
  \cmd{section}, \cmd{subsection}, \cmd{subsubsection}, \cmd{paragraph}, \cmd{subparagraph}, \cmd{part}, \cmd{appendix}, \cmd{tableofcontents}, \cmd{listoffigures}, and \cmd{listoftables}.
\end{center}
The article-style \texttt{abstract}, \texttt{titlepage}, and \texttt{theindex} environments are disabled too.
Use \cmd{makesection}, \cmd{makefoil}, and \cmd{makeappendix} instead.
...
Use \cmd{makereference} rather than calling \cmd{bibliography} directly.
